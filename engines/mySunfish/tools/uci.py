...
def go_loop_ret(searcher, hist, stop_event, rtime=10, max_depth=0, debug=False):
    

    start = time.time()
    best_move = None
    for depth, gamma, score, move in searcher.search(hist,rtime):
        # Our max_depth implementation is a bit wasteful.
        # We never know when we've seen the last at a certain depth
        # before we get to the next one
        if depth - 1 >= max_depth:
...
# Old tools stuff
def bestMove(sunfish_module, startpos,rtime):
    global sunfish
    sunfish = sunfish_module
